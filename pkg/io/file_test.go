...
func TestCreateFileIfNotExist(t *testing.T) {
	...
	require.NoError(t, err)
	expectedFilePath := filepath.Join(tmpDir, "my-service", "si-migrator.yml")
	type args struct {
		f string
	}
	tests := []struct {
...
func TestGetOrgSpace(t *testing.T) {
	pwd, err := os.Getwd()
	require.NoError(t, err)
	f := filepath.Join(pwd, "testdata", "cloudfoundry", "test-app", "mysqldb.yml")
	type args struct {
		path string
	}
	tests := []struct {
...
func TestTarUntar(t *testing.T) {
	tarTmpDir, err := os.MkdirTemp("", "")
	require.NoError(t, err)
	tarFilePath := filepath.Join(tarTmpDir, "some.tar")
	tarFile, err := CreateFileIfNotExist(tarFilePath)
	require.NoError(t, err)

	txtTmpDir, err := os.MkdirTemp("", "")
	require.NoError(t, err)
	tmpFilePath := filepath.Join(txtTmpDir, "some.txt")
	tmpFile, err := CreateFileIfNotExist(tmpFilePath)
	require.NoError(t, err, fmt.Sprintf("failed to create file '%s'", tmpFilePath))
	_, _ = tmpFile.WriteString("some text")
	_ = tmpFile.Close()
	...
	if err := Tar(tmpFilePath, tarFile); err != nil {
		t.Errorf("Tar() error = %v", err)
		require.NoError(t, err, fmt.Sprintf("failed to create tar file '%s'", tarFilePath))
	}
	_ = tarFile.Close()
	...
	if err := Untar(tarTmpDir, tarFile); err != nil {
		t.Errorf("Untar() error = %v", err)
		require.NoError(t, err, fmt.Sprintf("failed to untar file '%s'", tarFilePath))
	}
}
