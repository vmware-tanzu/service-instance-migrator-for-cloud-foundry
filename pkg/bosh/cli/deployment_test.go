...
var _ = Describe("Deployment", func() {
	...
	{"name": "dep", "releases":[{"name":"rel","version":"ver"}]}
		...
		XIt("returns an error for invalid release versions", func() {
			server.AppendHandlers(
				ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/deployments"),
					ghttp.RespondWith(http.StatusOK, `[
	...
	{"name": "dep", "stemcells":[{"name":"rel","version":"ver"}]}
		...
		XIt("returns an error for invalid stemcell versions", func() {
			server.AppendHandlers(
				ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/deployments"),
					ghttp.RespondWith(http.StatusOK, `[
	...
	Describe("FetchLogs", func() {
		...
		XIt("returns logs result for all deplotment", func() {
			ConfigureTaskResult(
				ghttp.CombineHandlers(
		...
		XIt("is able to apply filters and fetch agent logs", func() {
			ConfigureTaskResult(
				ghttp.CombineHandlers(
		...
		XIt("returns error if task response is non-200", func() {
			ConfigureTaskResult(
				ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/deployments/dep/jobs/job/id/logs", "type=job"),
					ghttp.VerifyBasicAuth("username", "password"),
	...
	Describe("EnableResurrection", func() {
		XIt("enables resurrection for all instances and returns without an error", func() {
			server.AppendHandlers(
				ghttp.CombineHandlers(
		...
		XIt("disables resurrection for all instances and returns without an error", func() {
			server.AppendHandlers(
				ghttp.CombineHandlers(
					ghttp.VerifyRequest("PUT", "/deployments/dep/jobs/job/id/resurrection"),
					ghttp.VerifyBasicAuth("username", "password"),
	...
	Describe("Ignore", func() {
		XIt("for an single instance, ignore instance returns without an error", func() {
			server.AppendHandlers(
				ghttp.CombineHandlers(
		...
		XIt("unignores for an instance and returns without an error", func() {
			server.AppendHandlers(
				ghttp.CombineHandlers(
					///:deployment/instance_groups/:instancegroup/:id/ignore
					ghttp.VerifyRequest("PUT", "/deployments/dep/instance_groups/ig_name/id/ignore"),
	...
	Describe("converge job states", func() {
		...
		for state, stateFunc := range states {
			...
			Describe(fmt.Sprintf("change state to '%s'", state), func() {
				XIt("changes state for specific instance", func() {
				...
				XIt("changes state for the whole deployment", func() {
				...
				XIt("changes state for all indicies of a job", func() {
				...
				XIt("changes state with canaries and max_in_flight set", func() {
					canaries := "50%"
					maxInFlight := "6"
				...
				if state == "recreate" {
					XIt("changes state with dry run", func() {
					...
					XIt("changes state with fix", func() {
				...
				if state != "started" {
					XIt("changes state with skipping drain and forcing", func() {
						slug = NewAllOrInstanceGroupOrInstanceSlug("", "")
						force = true
	...
	Describe("no-converge job actions", func() {
		...
		Describe("stop", func() {
			...
			XIt("accepts skip drain and hard", func() {
				slug := NewAllOrInstanceGroupOrInstanceSlug("my-ig", "id")
		...
		Describe("recreate", func() {
			...
			XIt("accepts skip drain and fix", func() {
				slug := NewAllOrInstanceGroupOrInstanceSlug("my-ig", "id")
	...
	Describe("ExportRelease", func() {
		...
		XIt("returns exported release result", func() {
			reqBody := `{
"deployment_name":"dep",
"jobs":[{"name":"fake-job"}],
"release_name":"rel",
...
"stemcell_version":"2"
		...
		XIt("returns error if response cannot be unmarshalled", func() {
	...
	Describe("Update", func() {
		XIt("succeeds updating deployment", func() {
			ConfigureTaskResult(
				ghttp.CombineHandlers(
		...
		XIt("succeeds updating deployment with recreate, recreate_persistent_disks, fix and skip drain flags", func() {
			ConfigureTaskResult(
				ghttp.CombineHandlers(
		...
		XIt("succeeds updating deployment with canaries and max-in-flight flags", func() {
			canaries := "100%"
		...
		XIt("succeeds updating deployment with dry-run flags", func() {
			dryRun := true
		...
		XIt("succeeds updating deployment with diff context values", func() {
	...
	Describe("Delete", func() {
		...
		XIt("succeeds deleting with force flag", func() {
		...
		XIt("succeeds even if error occurrs if deployment no longer exists", func() {
		...
		XIt("returns delete error if listing deployments fails", func() {
			AppendBadRequest(ghttp.VerifyRequest("DELETE", "/deployments/dep"), server)
		...
		XIt("returns delete error if response is non-200 and deployment still exists", func() {
			AppendBadRequest(ghttp.VerifyRequest("DELETE", "/deployments/dep"), server)
	...
	Describe("AttachDisk", func() {
		XIt("calls attachdisk director api", func() {
			ConfigureTaskResult(
				ghttp.CombineHandlers(
		...
		XIt("encodes url correctly", func() {
		...
		XIt("calls attachdisk director api with disk_properties if not empty", func() {
			ConfigureTaskResult(
				ghttp.CombineHandlers(
	...
	Describe("Variables", func() {
		XIt("returns the list of all variables", func() {
			server.AppendHandlers(
				ghttp.CombineHandlers(
		...
		XIt("returns an empty list if there are no variables", func() {
			server.AppendHandlers(
				ghttp.CombineHandlers(
					ghttp.VerifyRequest("GET", "/deployments/dep/variables"),
					ghttp.VerifyBasicAuth("username", "password"),
	...
	Describe("using a director with context", func() {
		...
		XIt("adds context to request headers", func() {
			ConfigureTaskResult(
				ghttp.CombineHandlers(
					ghttp.VerifyRequest("POST", "/deployments", ""),
					ghttp.VerifyBasicAuth("username", "password"),
